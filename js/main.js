define(['jsPDF', 'moment',
    ...
    $('body').on('click', '#main .build-report', function (e, eventData) {
        ...
        initReportTimer();
        refresher = setInterval(function(){ reportTimer() }, sixHours);
    });
    ...
    $('body').on('click', '#dropdown_dashboard li', function (e, eventData) {
        ...
        var searchString = from + '/' + to + '/' + formatDate(startDate) + '/' + formatDate(endDate);
    ...
    function generatePriceDiffSABarChart(data){
        ...
        if(nd.length){
            ...
            chartNegative = new Chart(ctxNegative).Bar(chartData, $.extend({scaleBeginAtZero: false}, chartConfigScroll));
