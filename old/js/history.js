define(['jsPDF', 'moment', 'chart1'] , function (jsPDF, moment) {
    ...
    $(function () {
        ...
        $('body').on('click', '#history .build-report', function (e) {
            e.preventDefault();
            e.stopPropagation();

            var sd = $('#history-range-start-date').val();
            var ed = $('#history-range-end-date').val();
            var od = $('#history-outbound-date').val();
            ...
            var endpoint = '//api1.webjet.com/metashopper/rs/airdata/history/' + from + '/' + to;
            endpoint += '/' + formatDate(outboundDate) + '/period/' + formatDate(startDate) + '/' + formatDate(endDate);
